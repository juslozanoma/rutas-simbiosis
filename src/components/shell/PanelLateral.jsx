/**
 * src/components/shell/PanelLateral.jsx
 * ---------------------------------------------------------------------------
 * Panel lateral fijo de escritorio / contenido móvil (aside.side-panel)
 * convertido a React como cáscara estática: mismo DOM con los mismos ids y
 * clases para que la lógica vanilla (panel.js, combosApp.js, escalas.js,
 * rutas.js, tour.js, descubre.js, altimetriaApp.js…) siga enlazando igual.
 *
...
function PanelLateral() {
  return (
    <aside className="side-panel">
      ...
      {/* Pestañas del panel (escritorio) */}
      <div className="panel-tab-bar" id="panel-tab-bar">
        <button type="button" id="btn-tab-panel-ruta" className="panel-tab panel-tab--active"><span id="btn-tab-panel-ruta-label">Ruta</span> <span className="tab-icon tab-icon--sign-post tab-icon--16" aria-hidden="true" id="ico-tab-ruta-desktop"></span></button>
        <button type="button" id="btn-anadir-ruta-desktop" className="panel-tab panel-tab--anadir" title="Añadir una nueva ruta" aria-label="Añadir una nueva ruta" hidden>
          <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" aria-hidden="true"><path d="M12 5v14M5 12h14" /></svg>
        </button>
        <button type="button" id="btn-cerrar-rutas-archivo-desktop" className="panel-tab panel-tab--cerrar-rutas" title="Cerrar rutas subidas y volver al menú normal" aria-label="Cerrar rutas subidas y volver al menú normal">
          <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="#d64545" strokeWidth="3" strokeLinecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18" /></svg>
        </button>
        <button type="button" id="btn-tab-panel-descubre" className="panel-tab">Descubre Colombia <span className="tab-icon tab-icon--colombia tab-icon--18" aria-hidden="true" id="ico-descubre-tab-desktop"></span><span className="badge-count mono" id="sitios-contador-tab-desktop" hidden>0</span></button>
        <button type="button" id="btn-reiniciar-pc" className="panel-tab panel-tab--reiniciar" title="Reiniciar desde cero (borra las rutas subidas y el viaje)" aria-label="Reiniciar desde cero">
          <span className="panel-tab--reiniciar__icon" aria-hidden="true"></span>
        </button>
      </div>

      <section className="panel-paradas" id="panel-paradas" hidden>
        <select id="filtro-municipios-departamento" className="filtro-municipios-departamento" hidden aria-label="Filtrar municipios por departamento">
          <option value="">Filtrar por departamento…</option>
        </select>
        <ul className="paradas-lista" id="paradas-lista"></ul>
      </section>

      <button type="button" id="btn-agregar-intermedio" className="btn-agregar-intermedio" title="Agregar parada" aria-label="Agregar parada">Añadir parada</button>

      {/* Ruta: origen/destino + paradas + monalisa */}
      <section className="panel-locate" id="panel-locate">
        <div className="panel-locate__row" id="row-origen">
          <div className="combo" data-combo="origen">
            <input type="search" className="combo__trigger" id="origen-input" placeholder="Origen" autoComplete="one-time-code" autoCorrect="off" autoCapitalize="off" spellCheck="false" aria-label="Seleccionar municipio de origen" />
            <svg className="combo__chevron" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M6 9l6 6 6-6" /></svg>
          </div>
          <button type="button" id="btn-agregar-escala" className="icon-btn" title="Agregar pueblo intermedio" aria-label="Agregar pueblo intermedio">
            <svg className="icon-btn__icon" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M12 5v14M5 12h14" /></svg>
          </button>
        </div>
        <div className="panel-locate__escalas" id="panel-escalas"></div>
        <div className="panel-locate__row" id="row-destino">
          <div className="combo" data-combo="destino">
            <input type="search" className="combo__trigger" id="destino-input" placeholder="Destino" autoComplete="one-time-code" autoCorrect="off" autoCapitalize="off" spellCheck="false" aria-label="Seleccionar municipio de destino" />
            <svg className="combo__chevron" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M6 9l6 6 6-6" /></svg>
          </div>
          <button type="button" id="btn-aereo" className="icon-btn icon-btn--aereo" title="Calcular ruta en avión" aria-label="Calcular ruta en avión" disabled>
            <svg viewBox="0 0 24 24" width="20" height="20" fill="#ffffff"><path d="M11.92,19.58,15.84,14H20a2,2,0,0,0,0-4H15.84L11.92,4.42A1,1,0,0,0,11.11,4h-.93a1,1,0,0,0-1,1.16L10,10H6.38L4.68,8.29A1.05,1.05,0,0,0,4,8H3a1,1,0,0,0-.89,1.45L3.38,12,2.11,14.55A1,1,0,0,0,3,16H4a1.05,1.05,0,0,0,.71-.29L6.38,14H10l-.81,4.84a1,1,0,0,0,1,1.16h.93A1,1,0,0,0,11.92,19.58Z" /></svg>
          </button>
          <button type="button" id="btn-calcular" className="icon-btn icon-btn--accent" title="Calcular ruta usando cualquier medio" aria-label="Calcular ruta usando cualquier medio" disabled>
            <svg className="icon-btn__icon" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#ffffff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M13 5l7 7-7 7" /></svg>
          </button>
        </div>
        <button type="button" id="btn-mostrar-sitios" className="btn-mostrar-sitios-cercanos" disabled hidden>Mostrar sitios turísticos cercanos</button>
      </section>

      {/* Tour: seleccionar destinos sin ruta, solo el listado en Descubre */}
      <section className="panel-tour" id="panel-tour" hidden>
        <div className="combo" data-combo="tour">
          <input type="search" className="combo__trigger" id="tour-input" placeholder="Seleccionar un destino" autoComplete="one-time-code" autoCorrect="off" autoCapitalize="off" spellCheck="false" aria-label="Seleccionar un destino" />
          <svg className="combo__chevron" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M6 9l6 6 6-6" /></svg>
        </div>
        {/* Barra de distancia del tour: aparece bajo el cuadro tras elegir el
             primer destino. */}
        <div className="filter-row filter-row--tour-distancia" id="filtro-distancia-tour-row" hidden>
          <div className="filter-row__field">
            <div className="filter-row__range">
              <span className="filter-row__thumb-val" id="filtro-distancia-tour-thumb">30 km</span>
              <input type="range" id="filtro-distancia-tour" min="1" max="30" value="30" step="1" />
            </div>
          </div>
        </div>
        <ul className="tour-destinos-lista" id="tour-destinos-lista"></ul>
      </section>

      <div className="btn-acciones-ruta">
        <button type="button" id="btn-subir-ruta-propia" className="btn-subir-ruta-propia" title="Subir tu propia ruta (KML o GPX)" aria-label="Subir tu propia ruta (KML o GPX)">Subir tu propia ruta</button>
        <button type="button" id="btn-iniciar-tour" className="btn-iniciar-tour" title="Tour personalizado por destinos" aria-label="Tour personalizado por destinos" aria-pressed="false">Tour personalizado</button>
      </div>

      <div id="loading-ruta" className="loading-ruta" hidden>
        ...
        <span className="loading-ruta__msg">Calculando la mejor ruta…</span>
      </div>

      {/* Descubre Colombia */}
      <div className="panel-descubre-actions" id="panel-descubre-actions" hidden>
        <button type="button" id="btn-descubre-categorias" className="descubre-btn">Categorías</button>
        <button type="button" id="btn-descubre-ordenar" className="descubre-btn">Ordenar</button>
        <button type="button" id="btn-descubre-desvios" className="descubre-btn">Filtrar</button>
        <button type="button" id="btn-descubre-visibles-btn" className="descubre-btn descubre-btn--round" title="Ver solo sitios visibles en el mapa" aria-label="Ver solo sitios visibles en el mapa" aria-pressed="false">
        ...
        <div className="descubre-dropdown" id="descubre-dropdown-categorias" hidden>
          <div className="panel-categorias__grid" id="categorias-grid"></div>
        </div>
        <div className="descubre-dropdown" id="descubre-dropdown-ordenar" hidden>
          <button type="button" id="btn-descubre-orden-origen" className="descubre-dropdown__item">Desde Origen ↓</button>
          <button type="button" id="btn-orden-dir" className="descubre-dropdown__item">Orden ascendente A-Z</button>
          <button type="button" id="btn-orden-dist" className="descubre-dropdown__item">Menor distancia</button>
        </div>
        <div className="descubre-dropdown" id="descubre-dropdown-desvios" hidden>
          <div className="filter-row">
            <input type="checkbox" id="check-distancia" className="filter-row__check" />
            <span className="filter-row__label-text">Distancia</span>
            <div className="filter-row__field">
              <div className="filter-row__range">
            ...
            <button type="button" id="btn-aplicar-distancia" className="icon-btn" title="Recalcular filtro de distancia" aria-label="Recalcular filtro de distancia" disabled>
              <span className="icon-btn__icon icon-btn__replay" aria-hidden="true"></span>
              <span className="icon-btn__spinner" aria-hidden="true"></span>
            </button>
          </div>
          <div className="filter-row">
            <input type="checkbox" id="check-tiempo" className="filter-row__check" />
            <span className="filter-row__label-text">&nbsp;Tiempo</span>
            <div className="filter-row__field">
              <div className="filter-row__range">
      ...
      <div id="loading-sitios" className="loading-sitios" hidden>
        <span className="loading-sitios__msg">Cargando lugares cercanos…</span>
        <svg className="spinner-bike" viewBox="0 0 48 30">
          <g fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1">
            <g transform="translate(9.5,19)">
              <circle className="spinner-bike_tire" r="9" strokeDasharray="56.549 56.549"></circle>
              <g className="spinner-bike_spokes-spin" strokeDasharray="31.416 31.416" strokeDashoffset="-23.562">
                <circle className="spinner-bike_spokes" r="5"></circle>
                <circle className="spinner-bike_spokes" r="5" transform="rotate(180,0,0)"></circle>
              </g>
            </g>
            <g transform="translate(24,19)">
              <g className="spinner-bike_pedals-spin" strokeDasharray="25.133 25.133" strokeDashoffset="-21.991" transform="rotate(67.5,0,0)">
                <circle className="spinner-bike_pedals" r="4"></circle>
                <circle className="spinner-bike_pedals" r="4" transform="rotate(180,0,0)"></circle>
              </g>
            </g>
            ...
            <polyline className="spinner-bike_seat" points="14 3,18 3" strokeDasharray="5 5"></polyline>
            <polyline className="spinner-bike_body" points="16 3,24 19,9.5 19,18 8,34 7,24 19" strokeDasharray="79 79"></polyline>
            <path className="spinner-bike_handlebars" d="m30,2h6s1,0,1,1-1,1-1,1" strokeDasharray="10 10"></path>
            <polyline className="spinner-bike_front" points="32.5 2,38.5 19" strokeDasharray="19 19"></polyline>
          </g>
        </svg>
      </div>

      <section className="panel-sites" id="panel-sites" hidden>
        <div className="panel-sites__header">
          <span className="badge-count mono" id="sitios-contador">0</span>
          <span className="badge-count mono" id="sitios-frontera-contador">0</span>
        </div>
        <div className="panel-sites__scroll">
          <p className="empty-state" id="sitios-vacio"></p>
          <ul className="sitios-lista" id="sitios-lista" hidden></ul>
        </div>
        <div className="panel-sites__busqueda">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="7" /><path d="M21 21l-4.35-4.35" /></svg>
          <input type="search" id="buscar-sitios" className="panel-sites__buscar" placeholder="Buscar en la lista…" autoComplete="off" aria-label="Buscar sitios en la lista" />
        </div>
      </section>
      ...
      <div id="altimetria-panel" className="altimetria-panel" hidden>
        <div className="altimetria__header">
          <div className="altimetria__titulo-grupo">
            <span className="altimetria__title">Perfil</span>
            <button type="button" id="btn-comparar-altimetria-panel" className="altimetria__vs" title="Comparar dos sitios del perfil" aria-label="Comparar dos sitios del perfil">VS</button>
          </div>
          <div className="altimetria__segmentos" id="altimetria-segmentos-panel" hidden></div>
          <span className="altimetria__hover-info">
            <span id="altimetria-dist-panel"></span>
            <span id="altimetria-alt-panel"></span>
          </span>
          <button type="button" id="btn-seguimiento-altimetria-panel" className="altimetria__seguimiento" title="Activar o desactivar el seguimiento" aria-pressed="true"><img src="/rutas-simbiosis/icons/scope.svg" alt="" width="12" height="21" /><span className="altimetria__seguimiento-label">Seguimiento</span></button>
        </div>
...
export default memo(PanelLateral);